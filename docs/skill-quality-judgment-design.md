...
页面把 `not-publishable` 表达为“当前证据不可发布”，并固定说明这不表示技能无效或任务失败。`shared` 过滤显示为“共享参与仅作上下文”，不复用正式快照缺失或技能质量失败的文案。阻断原因由服务端投影为受控中文标题和下一步动作，浏览器只渲染投影结果，不用内部 reason code 推断用户语义。

目录以当前启用技能范围为准。已退出此范围的旧详情链接返回明确的范围提示和返回目录动作，不把它伪装为技能质量结论或普通“对象不存在”。
