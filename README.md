...
- 将技能先安装到项目内 `skills-library/`，再按需复制到 `C:\Users\user\.codex\skills` 启用。
- 点击同步时扫描 `C:\Users\user\.codex\skills`，发现不在项目库中的额外技能会自动复制到 `skills-library/` 并登记为“本机纳管”；普通页面加载和 `GET /api/state` 只读取状态，不执行纳管写入。同步和安装完成后，未填写分类的技能会自动调用本机 `codex exec` 识别分类。
- 在 `data/skills-registry.json` 记录技能来源、分类、标签、依赖、启用状态、备注和同步时间。
...
- `skills-library/`：项目纳管的技能库。
- `data/skills-registry.json`：技能来源、分类、依赖、启用状态和备注。
...
`GET /api/state` 用于只读状态展示，会扫描项目库和当前 Codex skills 目录生成页面数据，但不会复制目录、写入 registry 或追加审计日志。需要自动纳管 `.codex/skills` 中额外技能时，点击页面顶部同步按钮或调用 `POST /api/sync`；该操作会把额外技能复制到 `skills-library/` 并记录审计日志，并在同步后尝试为仍是“未分类”的技能自动分类。
...
这些字段只写入 `data/skills-registry.json` 的 `localized` 元数据，不会写回 `skills-library/<skill>/SKILL.md`，也不会写回 `C:\Users\user\.codex\skills\<skill>\SKILL.md`。左侧“中文/原文”分段控件只改变管理台展示；启用到 Codex 的 skill 仍然保持原始文件内容。
...
- 真实使用证据来自结构化会话事件里的工具调用，例如读取 `C:\Users\user\.codex\skills\<skill>\SKILL.md`、`skills-library\<skill>\SKILL.md` 或插件缓存中的 `skills\<skill>\SKILL.md`。
...
系统技能位于 `C:\Users\user\.codex\skills\.system`，页面只展示，不允许停用。停用普通技能前需要二次确认；确认后只会删除 `C:\Users\user\.codex\skills\<skill-name>` 下的启用副本，项目库 `skills-library/<skill-name>` 会保留。
