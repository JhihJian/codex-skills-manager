...
工作台默认进入“技能质量”目录，只展示当前正确启用、非系统、非缺失，且 `.codex/skills` 启用副本 SHA 与历史 `valid + business-use + loaded` 调用精确匹配的技能版本。未启用、缺失、未加载、等待结果、失败加载、维护调用和历史旧版本保留在加载检测与审计中，不进入质量分析。目录与详情支持观察状态、参与关系、任务类型、来源和时间范围筛选，并在 URL 中保留当前范围。来自技能管理详情的入口先按技能名筛选当前启用版本的观察记录。
